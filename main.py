...
if "current_game" in st.session_state:
    if st.button("🎤 Launch AI Host Mode", type="primary", use_container_width=True):
        ...
        if "host_messages" not in st.session_state:
            ...
            st.session_state.host_messages.append({"role": "host", "content": f"Welcome to {stories[st.session_state.current_game['story_id']]['title']}! I am your AI Host for tonight's mystery."})
...
if st.session_state.get("host_mode", False):
    ...
    user_input = st.text_input("What should the host say or respond to?")
    
    if st.button("Send", type="primary"):
        if user_input:
            st.session_state.host_messages.append({"role": "player", "content": user_input})
            st.session_state.host_messages.append({"role": "host", "content": "Interesting question... Let me think about that in character."})
            st.rerun()

    st.info("Note: Full AI responses will be added once the openai package is installed.")
